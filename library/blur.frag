...
vec4 blur(sampler2D image, vec2 uv, int samples, vec2 resolution, bool horizontal)
{
    vec4 accum = vec4(0.0);
...
vec4 blurY(sampler2D image, vec2 uv, int samples, vec2 resolution)
{
    return blur(image, uv, samples, resolution, false);
}

vec4 blurX(sampler2D image, vec2 uv, int samples, vec2 resolution)
{
